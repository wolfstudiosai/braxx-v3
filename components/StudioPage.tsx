...
export const StudioPage: React.FC<StudioPageProps> = ({ addToCart }) => {
  const prototypeData = {
    id: 999,
    name: 'S-PROTOTYPE',
    price: 8500,
    category: 'STUDIO',
    image: '',
    description: 'Custom studio R&D prototype.'
  };
  ...
  return (
    <div className="bg-[#d4d4d4] min-h-screen selection:bg-black selection:text-[#e2ff4a]">
      ...
      <section className="relative w-full h-[60vh] lg:h-[75vh] overflow-hidden bg-black">
        ...
        >
          <source src="https://player.vimeo.com/external/403487343.sd.mp4?s=0894e63777591873911029c3620317e883216894&profile_id=165&oauth2_token_id=57447761" type="video/mp4" />
        </video>
      ...
      <div className="max-w-[1600px] mx-auto px-8 lg:px-24">
        ...
        <section className="mb-32">
          <div className="group relative w-full aspect-video lg:h-[800px] bg-white/20 border border-black/5 overflow-hidden flex items-center justify-center">
             {/* Background Tech Grid */}
             <div className="absolute inset-0 opacity-[0.03] pointer-events-none" style={{ backgroundImage: `linear-gradient(#000 1px, transparent 1px), linear-gradient(90deg, #000 1px, transparent 1px)`, backgroundSize: '40px 40px' }}></div>
             
             {/* Center Piece */}
             <div className="relative w-full h-full flex items-center justify-center">
                <BikeOutline className="scale-[1.6] lg:scale-[1.8] opacity-90 transition-all duration-1000 group-hover:scale-[1.9]" />
                
                {/* Minimal Overlay Data */}
                <div className="absolute top-10 left-10 flex flex-col gap-1">
                   <span className="text-[8px] font-black tracking-widest text-black/20 uppercase">Core Architecture</span>
                   <span className="text-xl font-black italic uppercase">BRAXX S-LEVEL</span>
                </div>

                <div className="absolute bottom-10 right-10 text-right space-y-4">
                   <div className="space-y-1">
                      <span className="block text-[7px] font-black text-black/30 uppercase tracking-widest">Aero Efficiency</span>
                      <div className="w-48 h-[1px] bg-black/5">
                        <div className="w-[88%] h-full bg-black"></div>
                      </div>
                   </div>
                   <div className="space-y-1">
                      <span className="block text-[7px] font-black text-black/30 uppercase tracking-widest">Thermal Mass</span>
                      <div className="w-48 h-[1px] bg-black/5">
                        <div className="w-[42%] h-full bg-black"></div>
                      </div>
                   </div>
                </div>
             </div>

             {/* Functional Corner Widgets (Minimal) */}
             <button className="absolute top-10 right-10 p-2 text-black/20 hover:text-black transition-colors"><Maximize2 size={16}/></button>
          </div>
          ...
          <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-8">
             ...
             <button 
                onClick={() => addToCart(prototypeData)}
                className="w-full h-full flex items-center justify-center bg-white text-black border border-black/10 rounded-3xl font-black uppercase tracking-[0.3em] text-xs hover:bg-[#e2ff4a] transition-all"
             >
                Initialize Build Protocol // Add to Cart
             </button>
          </div>
        </section>

        {/* LAB REPORTS: Grid view of technical blog posts */}
        <section className="mb-32">
          <div className="flex items-baseline justify-between mb-12 border-b border-black/10 pb-6">
             <div className="space-y-1">
                <span className="text-black/30 text-[9px] font-black uppercase tracking-[0.5em] block">Archived Data</span>
                <h3 className="text-4xl font-light italic tracking-tighter uppercase text-black">Laboratory Reports</h3>
             </div>
             <button className="text-[10px] font-black uppercase tracking-widest hover:text-white transition-colors">Technical Database</button>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {technicalPosts.map((post) => (
              <div key={post.id} className="group cursor-pointer space-y-6">
                <div className="relative aspect-video rounded-3xl overflow-hidden bg-black/5">
                  <img 
                    src={post.image} 
                    alt={post.title} 
                    className="w-full h-full object-cover grayscale opacity-80 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-700" 
                  />
                  <div className="absolute top-4 left-4 bg-black/50 backdrop-blur-md px-3 py-1 rounded-full text-[8px] font-black text-white uppercase tracking-widest">
                    {post.category}
                  </div>
                </div>
                <div className="space-y-3">
                  <div className="flex items-center gap-4 text-black/30 text-[8px] font-black uppercase tracking-widest">
                    <span>{post.date}</span>
                    <span className="w-8 h-px bg-current"></span>
                    <span className="flex items-center gap-1"><Clock size={10}/> 5 MIN READ</span>
                  </div>
                  <h4 className="text-xl font-black italic tracking-tight uppercase leading-none group-hover:text-white transition-colors">
                    {post.title}
                  </h4>
                  <p className="text-[10px] font-bold text-black/50 leading-relaxed uppercase tracking-tight line-clamp-2">
                    {post.excerpt}
                  </p>
                  <div className="pt-2 flex items-center gap-2 text-[9px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">
