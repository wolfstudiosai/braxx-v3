...
import {
  ...
  Cpu, Layers, Disc, Wind, ArrowRight, Calendar as CalendarIcon, ArrowLeft, Plus, PlayCircle
} from 'lucide-react';
...
export function ProductPageClient({ product, initialVariantIndex }: ProductPageClientProps) {
  ...
  // Horizontal specs for the new spec row
  ...
  const tutorials = [
    {
      id: 0,
      sub: "Urban Operations",
      title: "MASTER THE SILENCE",
      desc: "Tactical stealth protocols for metropolitan environments.",
      src: "https://images.unsplash.com/photo-1558980331-0672ff794669?q=80&w=2400&auto=format&fit=crop"
    },
    {
      id: 1,
      sub: "Field Maintenance",
      title: "BATTERY SWAP PROTOCOL",
      desc: "Rapid energy cell exchange procedure under 30 seconds.",
      src: "https://images.unsplash.com/photo-1558981403-c5f9899a28bc?q=80&w=2400&auto=format&fit=crop"
    },
    {
      id: 2,
      sub: "Performance Tuning",
      title: "SUSPENSION CALIBRATION",
      desc: "Adjusting rebound and compression for variable terrain.",
      src: "https://images.unsplash.com/photo-1558981285-6f0c94958bb6?q=80&w=2400&auto=format&fit=crop"
    },
    {
      id: 3,
      sub: "System Update",
      title: "V-CORE FIRMWARE 2.0",
      desc: "Overview of new torque mapping and regen braking features.",
      src: "https://images.unsplash.com/photo-1599819811279-d5ad9cccf838?q=80&w=2400&auto=format&fit=crop"
    }
  ];
  ...
  return (
    <div className="bg-[#d4d4d4] min-h-screen">
      ...
      {/* NEW HORIZONTAL SPEC ROW */}
      <div className="w-full bg-white border-y border-black/10 overflow-x-auto hide-scrollbar">
        <div className="max-w-[1920px] mx-auto px-8 lg:px-24 flex items-center justify-between min-w-max gap-16 py-8">
          <div className="flex items-center gap-4">
            <span className="text-[10px] font-black text-black uppercase tracking-[0.3em]">Technical Specs</span>
            <div className="h-8 w-px bg-black/10"></div>
          </div>
          {horizontalSpecs.map((spec, i) => (
            <div key={i} className="flex flex-col group cursor-default">
              <span className="text-[8px] font-black text-black/40 uppercase tracking-widest mb-1 group-hover:text-[#e2ff4a] transition-colors">{spec.label}</span>
              <span className="text-sm font-black italic uppercase text-black">{spec.val}</span>
            </div>
          ))}
          <div className="flex-1"></div>
          <button className="text-[9px] font-black uppercase tracking-widest hover:text-[#e2ff4a] transition-colors whitespace-nowrap">Download Full Sheet</button>
        </div>
      </div>

      {/* SECTION 3: PROPRIETARY TECH GRID - WHITE BACKGROUND WITH BORDERS */}
      <section className="bg-white w-full">
        <div className="max-w-[1920px] mx-auto">

          {/* Section Header */}
          <div className="p-12 md:p-16 border-b border-black/10">
            <span className="text-black/40 text-[10px] font-black uppercase tracking-[0.5em] block mb-4">The Architecture</span>
            <h2 className="text-black text-5xl md:text-7xl font-black italic uppercase tracking-tighter leading-none">
              ENGINEERED <br /> FOR <span className="text-transparent" style={{ WebkitTextStroke: '1px black' }}>SURVIVAL</span>
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              <div key={i} className={`group relative h-[500px] border-b border-black/10 flex flex-col justify-between p-12 overflow-hidden bg-white ${i !== 3 ? 'lg:border-r' : ''} ${i % 2 === 0 ? 'md:border-r' : ''} ${i < 2 ? 'lg:border-r' : ''}`}>
                {/* Background Image Layer */}
                <div className="absolute inset-0 z-0">
                  <img
                    src={tech.img}
                    alt={tech.title}
                    className="w-full h-full object-cover opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-out grayscale group-hover:grayscale-0"
                  />
                ...
                <div className="relative z-10 h-full flex flex-col justify-between pointer-events-none">
                  <div className="text-black group-hover:text-[#e2ff4a] transition-colors duration-300">
                    {tech.icon}
                  </div>

                  <div className="space-y-4">
                    <h3 className="text-3xl font-black italic tracking-tighter uppercase leading-none text-black group-hover:text-white transition-colors duration-300">
                      {tech.title}
                    </h3>
                    <p className="text-xs font-bold text-gray-400 group-hover:text-gray-200 transition-colors duration-300 uppercase tracking-wide leading-relaxed">
                      {tech.desc}
                    </p>
                  </div>
                </div>

                <div className="absolute top-8 right-8 z-10 opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-x-4 group-hover:translate-x-0 text-white">
                  <ArrowRight size={24} />
                </div>
              </div>
            ))}
        ...
        <div className="w-full lg:w-[400px] h-[40vh] lg:h-full flex flex-col border-b lg:border-b-0 lg:border-r border-white/10 overflow-y-auto hide-scrollbar z-20 bg-black shrink-0">
          ...
          {tutorials.map((t, i) => (
            <div
              ...
              className={`group relative p-8 border-b border-white/10 cursor-pointer transition-all duration-300 ${activeTutorial === i ? 'bg-white/10' : 'hover:bg-white/5'}`}
            >
              <div className="flex gap-4 items-center">
                <div className={`w-16 h-16 rounded-xl overflow-hidden border border-white/20 shrink-0 ${activeTutorial === i ? 'border-[#e2ff4a]' : ''}`}>
                  <img src={t.src} className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-opacity" />
                </div>
                <div className="space-y-2">
                  <span className={`text-[8px] font-black uppercase tracking-widest block transition-colors ${activeTutorial === i ? 'text-[#e2ff4a]' : 'text-white/40'}`}>
                    0{i + 1} // {t.sub}
                  </span>
                  <h4 className="text-sm font-black italic uppercase text-white leading-tight">{t.title}</h4>
                </div>
              </div>

              {activeTutorial === i && (
                <div className="absolute right-6 top-1/2 -translate-y-1/2 text-[#e2ff4a]">
                  <PlayCircle size={20} fill="currentColor" className="text-black" />
                </div>
              )}
            </div>
          ))}

          {/* Fills remaining space in sidebar */}
        ...
        <div className="flex-1 relative h-full bg-black flex items-center justify-center overflow-hidden">
          <div key={activeTutorial} className="absolute inset-0 z-0 animate-in fade-in duration-1000">
            <img
              ...
              alt={tutorials[activeTutorial].title}
          ...
          <div className="relative z-10 text-center space-y-8 px-12 max-w-4xl mx-auto animate-in slide-in-from-bottom-8 duration-700">
            ...
            <p className="text-white/60 font-bold uppercase tracking-widest text-xs max-w-md mx-auto">
              {tutorials[activeTutorial].desc}
            </p>
            <button className="mx-auto mt-12 bg-white text-black px-12 py-5 rounded-full font-black uppercase text-xs tracking-widest hover:bg-[#e2ff4a] transition-all transform hover:scale-110 shadow-2xl flex items-center gap-3">
              <PlayCircle size={16} /> Watch Briefing
            </button>
          </div>
        </div>
      </section>
